...
class ThumbnailTest(BaseTest):
    ...
    def testRGBA(self):
        # RGBA image
        rgba_pic_name = default_storage.path('sorl-thumbnail-test_rgba_source.png')
        Image.new('RGBA', PIC_SIZE).save(rgba_pic_name)
        self.images_to_delete.add(rgba_pic_name)
        # Create thumb and verify it's still RGBA
        rgba_thumb_name = default_storage.path('sorl-thumbnail-test_rgba_dest.png')
...
class DjangoThumbnailTest(BaseTest):
    def setUp(self):
        ...
        # basedir.
        self.sub_dir = default_storage.path('test_thumbnail')
        try:
    ...
    def testFilenameGeneration(self):
        ...
        expected = default_storage.path(basename)
        ...
        expected = (default_storage.path(basename) + '_240x120_'
        ...
        thumb = DjangoThumbnail(relative_source=self.pic_subdir,
                                requested_size=(240, 120))
        expected = os.path.join(os.path.basename(self.sub_dir), 'subdir',
                                basename)
        expected += '_240x120_q85.jpg'
        expected = default_storage.path(expected)
        self.verify_thumbnail((160, 120), thumb, expected_filename=expected)
        # Different prefix
    ...
    def testUnicodeName(self):
        ...
        unicode_path = default_storage.path(unicode_name)
        ...
        expected = default_storage.path(base_name + '_240x120_q85.jpg')
        self.verify_thumbnail((160, 120), thumb, expected_filename=expected)
