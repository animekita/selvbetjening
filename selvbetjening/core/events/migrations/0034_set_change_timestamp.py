...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for attendee in orm.Attend.objects.all():
            ...
            try:
                last_change = orm.AttendStateChange.objects.filter(attendee=attendee).order_by('-id')[0]
                attendee.changed = last_change.timestamp
                attendee.save()

            except IndexError:
                attendee.changed = attendee.registration_date
                attendee.save()
