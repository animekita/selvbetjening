...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for payment in orm['invoice.Payment'].objects.all():

            try:
                attendee = orm['events.Attend'].objects.filter(invoice=payment.invoice).latest('pk')
            except orm['events.Attend'].DoesNotExist:
                attendee = None

            p = orm['events.Payment'].objects.create(
                user=payment.invoice.user,
                attendee=attendee,
                amount=payment.amount,
                signee=payment.signee,
                note=payment.note,
                created_date=payment.created_date
            )

            p.created_date = payment.created_date
            p.save()
