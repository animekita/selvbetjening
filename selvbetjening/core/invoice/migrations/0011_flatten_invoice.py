...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for invoice in orm.Invoice.objects.all():
            ...
            latest = orm.InvoiceRevision.objects.filter(invoice=invoice).latest('id')
            invoice.updated_date = latest.created_date

            for line in latest.line_set.all():
                invoice.total_price += line.price

                orm.InvoiceLine.objects.create(invoice=invoice,
                                               description=line.description,
                                               group_name=line.group_name,
                                               price=line.price,
                                               managed=line.managed)

            first = orm.InvoiceRevision.objects.order_by('id').all()[0]
            invoice.created_date = first.created_date

            invoice.save()

        for revision in orm.InvoiceRevision.objects.all():

            for line in revision.line_set.all():
                revision.total_price += line.price
